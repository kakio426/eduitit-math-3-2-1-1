...
`방법 다시 보기`는 현재 화면 이름을 저장한 뒤 설명 화면을 복습 모드로 보여 주고, 버튼 문구를 `계속하기`로 바꿉니다. `계속하기`를 누르면 문제/보상/결과 상태를 초기화하지 않고 원래 화면으로 돌아옵니다. `처음부터`는 바로 리셋하지 않고 `처음부터 할까요?` 확인을 거친 뒤 첫 화면으로 돌아갑니다.
