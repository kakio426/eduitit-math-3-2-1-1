...
4. 바람: 한 문제 뒤 한 가지 바람만 보여 줍니다. 바람 종류마다 매스몬이 함께 들어간 보상 모달 이미지가 바뀝니다.
...
큰 이동 연출은 문제 풀이 화면에 넣지 않습니다. 정답 뒤 보상 모달에서 바람 장면을 보여 주고, 문제 화면은 계산과 선택지에 집중합니다. 상단 지도에는 `현재` 글자 대신 작은 냥냥몬 마커만 현재 섬으로 움직입니다. 보상 모달의 매스몬은 별도 PNG를 붙인 것이 아니라 각 바람 이미지 안에 함께 생성했습니다.
