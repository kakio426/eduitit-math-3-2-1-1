...
정답을 바로 고르면 선택지 색만 바꾸고 넘기지 않습니다. 1단계는 계산판이 `49 × 6 = 294`처럼 바뀌고 `맞았어요. 다음은 0 붙이기예요.`를 보여 준 뒤 2단계로 갑니다. 2단계는 `294 → 2,940`처럼 최종 답을 보여 주고, 보상 모달 전에 완성식이 한 번 더 보입니다.
...
- 힌트: 지금 단계 힌트 한 줄만 표시
- 보상 모달: 버튼을 빼고 보이는 문구는 한 개. 기본 바람은 `살랑 바람`으로 표시하고, 이동감은 텍스트나 별도 캐릭터 합성이 아니라 결과별 생성 이미지로 처리
- 실수 보상 문구: `길이 흔들렸어요`
...
바람 문구 수정 뒤에는 `.tmp-qa/lesson3-wind-wording/metrics.json`으로 1280x800, 1024x768 설명 화면과 `살랑 바람` 보상 모달을 다시 확인했습니다. 두 화면 크기 모두 옛 표현 0건, 텍스트 넘침 0건, Stage 밖 요소 0건으로 통과했습니다.
