...
`곱셈 상자런`은 3학년 2학기 1단원에서 다루는 세 자리 수 × 한 자리 수 계산을 짧고 반복적인 게임 흐름으로 연습하는 에듀잇티 수학 게임입니다. 학생은 문제를 맞히면 상자를 열고, 상자 안의 보상으로 점수가 크게 오르거나 내려갑니다. 최종 점수와 정답 수에 따라 서로 다른 매스몬을 얻습니다.
...
- 문제 은행: 조건을 만족하는 186개 후보에서 오늘의 도전 10문제 추출
- 오늘의 도전: 한국 날짜 기준 도전 코드로 문제와 보상 흐름 고정
- 라운드 길이: 10문제
- 입력 방식: 4지선다 선택
...
문제는 각 자리 숫자와 곱하는 수를 조합할 때 자리별 곱이 9를 넘지 않도록 만들어 받아올림이 생기지 않게 구성했습니다. 후보 문제는 총 186개이며, 한국 날짜 기준 오늘의 도전 코드로 문제 순서, 선택지, 상자 등급, 보상 흐름이 정해집니다. 같은 날에는 같은 조건으로 플레이할 수 있어 친구끼리 `오늘 점수 몇 점?`을 비교하기 쉽습니다.
...
정답을 맞히면 콤보 점수를 받고 색깔 상자가 준비됩니다. 연속으로 맞힐수록 기본 정답 점수가 `+100`, `+200`, `+300`, `+400`, `+500`까지 커집니다. 정답 상자는 일반, 반짝, 황금 3단계이며 황금 상자일수록 좋은 보상 확률이 조금 더 높습니다. 상자에서는 `+`, `-`, `×`, `÷`, `0`, `-를 +로` 같은 효과가 한 번 적용됩니다. 점수는 음수와 큰 수를 모두 허용합니다. 첫 화면과 결과 화면에는 날짜 기준 `오늘의 도전` 코드가 표시됩니다. 결과 화면에서는 정답 수와 점수에 맞춘 짧은 칭찬 문장을 함께 보여 줍니다.
...
게임 제목과 시작 버튼만 크게 보여 줍니다. 왼쪽 위에는 작게 `에듀잇티 수학 게임` 배지를 넣어 시리즈 흔적만 남기고, 학생의 주의를 시작 버튼으로 모았습니다.
