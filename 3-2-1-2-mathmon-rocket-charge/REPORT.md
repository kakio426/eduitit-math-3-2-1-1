...
설명 화면은 `tutorial-generated.webp`를 RasterStage 배경으로 사용합니다. 이미지에는 세로셈판, 로켓 연료칸, 연료 이벤트, 멀리 이어지는 우주 길을 텍스트 없이 보여 주고, 학생이 읽어야 할 제목, 3단계 문구, 시작 버튼은 HTML로 얹습니다.

설명 화면 문구는 다음만 보여 줍니다.

- 제목: 세로셈을 차례로 풀어요.
- 1단계: 문제를 풀면 연료가 변해요.
- 2단계: 연료가 많이 늘어나거나 갑자기 줄어들 수도 있어요.
- 3단계: 모인 연료에 따라 갈 수 있는 행성이 달라져요.
